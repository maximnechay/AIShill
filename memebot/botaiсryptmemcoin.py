...
USERNAMES = [
    "devinvest_hub",  # DEVINVEST (мой аккаунт)
    "DevInVestCoin",  # Мемкоин (мой аккаунт)
    "solana",  # Solana
    "rektcapital",  #
    "solana",  #
    "cobie",  #
    "cb_doge",  #
    "devdevinvest",  # DEVINVEST Developers (мой аккаунт)
    "CryptoDonAlt",  #
    "scottmelker",  #
    #
    "maximalpow",  # Max P (мой акк)
    "Crypto_Crib_",  #
    "AltcoinGordon",  #
    "WatcherGuru",  #
    "zerohedge",  #
    "i/communities/1699807431709041070",
    "i/communities/1473095538211885056",
    "i/communities/1847770491798638869",
    "maxxonchain",  # Maxim Nechay (мой аккаунт)
    "i/communities/1472225107389071364",  # Web 3.0
    "i/communities/1910972484549067167",  # Start Ups in Web 3
    "i/communities/1540105672263712768",  # Tokenization Metaverse & Web 3
    "i/communities/1849211810152628440",  # Web Design
]
...
class AutoCycleResponder:
    def __init__(self):
        ...
        self.cycle_interval_minutes = 25  # Увеличен интервал
        self.max_responses_per_cycle = 1  # Уменьшен лимит для стабильности
        self.max_daily_responses = 40  # Разумный лимит
        self.max_tweets_per_account = 4  # Меньше твитов для быстроты
    ...
    def get_tweet_style(self, tweet_text):
        """Классификация стиля твита с учётом мемов"""
    ...
    async def generate_response(self, tweet_text, username):
        """Асинхронная генерация ответа с учетом автора и стиля"""
        try:
            ...
            if username in ["elonmusk", "saylor"]:
                ...
                audience = "tech"
            elif username in ["cryptomeltdown", "degentrading", "rektcapital", "cobie"]:
                audience = "memecoin"
            else:
                audience = "crypto"

            # Автоматический подбор тона по стилю
            tone_map = {
                "degen": "witty",
                "shitpost": "sarcastic",
                "edgelord": "aggressive",
            }
            tone = tone_map.get(style, None)
                ...
                for i, article in enumerate(articles[:max_check]):
                    try:
                        ...
                        if await tweet_text_elem.count() > 0:
                            tweet_text = await tweet_text_elem.first.inner_text()
