...
from conversion_optimizer import ConversionOptimizer, EnhancedReplyEngine
...
USERNAMES = [
    # Мем-инфлюенсеры и дженераль дегены
    "Kevsznx",
    "mercularx",
    "henn100x",
    "ihateoop",
...
class RobustAutoResponder:
    def __init__(self):
        ...
        # Настройки циклов
        self.cycle_interval_minutes = 18
    ...
    async def generate_response(self, tweet_text, username, followers_count=0):
        try:
            if not self.enhanced_engine:
                base_engine = await self.get_reply_engine()
                self.enhanced_engine = EnhancedReplyEngine(
                    base_engine, self.conversion_optimizer
                )

            result = await self.enhanced_engine.generate_optimized_reply(
