...
class RobustAutoResponder:
    def __init__(self):
        ...
        # Настройки циклов
        self.cycle_interval_minutes = 21
        self.max_responses_per_cycle = 1
        self.max_daily_responses = 50
                ...
                for i, article in enumerate(articles[:max_check]):
                    try:
                        ...
                        if (
                            ...
                            # Проверяем, не был ли твит уже обработан
    ...
    def is_good_tweet_for_reply(self, tweet_text):
        """Улучшенная проверка, подходит ли твит для ответа"""
        text_lower = tweet_text.lower()

        # 1. СТРОГИЕ ИСКЛЮЧЕНИЯ (только очевидно плохие твиты)
        strict_bad_indicators = [
            # Спам и промо
            "join my",
            "get your",
            "claim now",
        ...
        if any(indicator in text_lower for indicator in strict_bad_indicators):
            return False

        # 2. РАСШИРЕННЫЕ КЛЮЧЕВЫЕ СЛОВА (более широкий охват)
        crypto_keywords = [
        ...
        # 3. КОНТЕКСТНЫЕ ИНДИКАТОРЫ (эмоциональная окраска)
        engaging_indicators = [
            # Вопросы и дискуссии
            "what do you think",
            "thoughts on",
        ...
        # 5. ПРОВЕРКА НА РЕЛЕВАНТНОСТЬ
        all_keywords = crypto_keywords + finance_keywords + tech_keywords
        has_keywords = any(keyword in text_lower for keyword in all_keywords)
        has_engaging_content = any(
            indicator in text_lower for indicator in engaging_indicators
        )

        # Проверка на числовые данные (регулярные выражения)
        import re

        has_numbers = bool(re.search(r"\d+[%$kmbx]|\d+\.\d+", text_lower))

        # 6. ДЛИНА И СТРУКТУРА (более гибкие требования)
        word_count = len(tweet_text.split())
        char_count = len(tweet_text)

        # Более гибкие требования к длине
        is_good_length = (10 <= char_count <= 600) and (  # Снижен минимум с 20 до 10
            word_count >= 2
        )  # Снижен минимум с 4 до 2

        # 7. СПЕЦИАЛЬНЫЕ СЛУЧАИ (известные авторы)
        known_crypto_accounts = [
            "elonmusk",
            "VitalikButerin",
            "cz_binance",
            "saylor",
        ...
        # Для известных аккаунтов - более мягкие требования
        is_from_known_account = (
            hasattr(self, "current_username")
            and self.current_username in known_crypto_accounts
        )
        ...
        # Для известных аккаунтов - достаточно одного условия
        if is_from_known_account:
            return has_keywords or has_engaging_content or has_numbers

        # Для остальных - нужно минимум 2 условия из 3
        relevance_score = sum([has_keywords, has_engaging_content, has_numbers])

        return relevance_score >= 1  # Снижен порог с 2 до 1
